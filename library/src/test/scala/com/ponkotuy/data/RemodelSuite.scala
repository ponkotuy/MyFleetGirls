...
class RemodelSuite extends FunSuite {
  test("success pattern") {
    ...
    val expected = Remodel(true, 2, 2, 0, Some(RemodelAfterSlot(46284, 2, true, 5)), Nil, false)
  ...
  test("success pattern with consume slotitem") {
    ...
    val expected = Remodel(true, 2, 2, 0, Some(RemodelAfterSlot(46185, 2, true, 9)), List(48952), false)
    common(jsonValue, postValue, expected)
  }
  ...
  test("success pattern with powerup slotitem") {
    ...
    val expected = Remodel(true, 120, 121, 10, Some(RemodelAfterSlot(46594, 121, false, 0)), List(198, 1642), true)
    common(jsonValue, postValue, expected)
  }
  ...
  test("failure pattern") {
    ...
    val expected = Remodel(false, 2, 2, 0, None, Nil, false)
    common(jsonValue, postValue, expected)
  }
